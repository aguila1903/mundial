...
if (isset($_REQUEST["name"])) {
    ...
    if ($name != "null" && $name != "") {
        if (strlen($name) > 264 || strlen($name) < 1) {
            $out{'response'}{'status'} = -1;
            $out{'response'}{'errors'} = array('name' => "Bitte einen Trainernamen mit max. 264 Zeichen eingeben.");
        ...
        $out{'response'}{'status'} = -1;
        $out{'response'}{'errors'} = array('name' => "Trainername fehlt!");
...
while (!$rs->EOF) {
    ...
    $value{$i}{"trainer_id"} = $rs->fields{'trainer_id'};
